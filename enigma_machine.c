...
int validate_options(char *options) { 
	...
	while (*opt){
		if(isdigit(*opt))
			if ((int) opt == 1 || (int) opt == 2 || (int) opt ==3)
			printf("you entered a text");	
				return (int) opt;
...
int *parse_rotor_indices(char *rotor_indices_line) {
   ...
   int i;
    int len = strlen(rotor_indices_line);
		...
		while (j < len)
 		 {
	  		...
	  		if (rotor_indices_line[j]>= '0' && rotor_indices_line[j]<= '8')
  			{
				ret[k]= rotor_indices_line[j]-48;
				//printf("%d", ret[k]);
